...
//------------------------------------------------------------------------------------------------//
    ...
    public static function getUserAjax($q, $is_tipo = false,$tipo = false)
    {
        // La respuesta sera en Formato JSON
        Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        $query = (new Query())
            ->select([
                "view_user.`id`",
                "`nombre_completo` AS `text`",
            ])
            ->from(self::tableName())
            ->andWhere(['like', 'nombre_completo', $q])
            ->orderBy('id desc')
            ->limit(50);
            ...
            if ($is_tipo && $tipo == User::TIPO_AGENTE) {
                $query->leftJoin("user_a_user","user_a_user.user_agente_id = view_user.id");
                $query->andWhere(['IS', 'user_a_user.user_agente_id', new \yii\db\Expression('null')]);
            }
    ...
    /*public static function getUserSucursalAjax($q)
    {
        // La respuesta sera en Formato JSON
        Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        $query = (new Query())
            ...
            ->from('user_sucursal')
            ->innerJoin('sucursal',['user_sucursal.sucursal_id' => 'sucursal.id'])
            ->andWhere(['user_sucursal.user_id',  $q])
