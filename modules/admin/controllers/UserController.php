...
class UserController extends \app\controllers\AppController
{
    ...
    public function actionUpdate($id)
    {
        ...
        $user->setPerfilesAsignarNames();
        $user->setSucursalesPermisosNames();
...
//------------------------------------------------------------------------------------------------//
    ...
    public function actionLogin()
    {
        ...
        //         ->setTo($user->email)
        //         ->setSubject('Notificación de inicio de sesión')
        //         ->setTextBody('Se ha iniciado sesión en su cuenta.')
        //         ->send();
        // } catch (\Exception $e) {
        //     Yii::error('Error al enviar notificación de inicio de sesión: ' . $e->getMessage());
        // }
...
//------------------------------------------------------------------------------------------------//
    ...
    public function actionUserAgentesSupervisorAjax($q = false,$is_tipo)
    {
        $request = Yii::$app->request;
        ...
        if ($request->validateCsrfToken() && $request->isAjax) {
            ...
            if ($is_tipo)
                $user = ViewUser::getUserAjax($text,true,$is_tipo);
            else
                $user = ViewUser::getUserAjax($text);
